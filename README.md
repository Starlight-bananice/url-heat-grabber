...
当前 Windows 版本：`0.5.2`；macOS 版本：`0.5.1`
...
- 上面的分隔线位于链接区和处理设置之间，向上或向下拖动可以调整链接输入区高度；
- 下面的分隔线位于处理按钮和运行日志之间，向上或向下拖动可以调整运行日志高度；
- 如果链接很多，可以把上方区域拉大；如果需要查看详细报错，可以把下方日志区域拉大。
...
Windows 版已取消“验证码辅助”和验证码账号配置，也不会读取账号密码或调用第三方打码服务。遇到平台验证码时，程序会按普通抓取失败处理。macOS 版仍保留原有配置入口。
