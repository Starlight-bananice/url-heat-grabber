...
当前 macOS 和 Windows 版本均为 `0.5.2`。

macOS `0.5.2` 移除了验证码辅助、验证码账号和密码输入；启动处理时会清理旧版遗留的本机 `yzm.txt` 账号文件。此版本同时包含对百度、网易、微博、微信、贴吧、B站、凤凰和快手页面结构变化的修复。Windows 安装包保持你已发布的 `0.5.2`，本次只追加 macOS 安装包。
...
快手也固定使用 `1` 个独立 worker，并在打开作品前建立匿名网页会话。所有平台合计最多同时使用 `4` 个浏览器，不会因新增快手 worker 提高总上限。
...
- 空白：页面正常打开；程序会填写当前能公开取得的互动数据；
- `已删除`：页面明确提示内容不存在，或文章短链已跳回频道首页；
- `需验证`：例如微信公众号要求完成环境验证，程序无法在无头浏览器里确认内容；
- `访问受限`：平台返回空白默认页，无法可靠确认作品及互动数；
- `处理失败`：浏览器会话或网络本次异常，可勾选“从上次进度继续”后重试。

`需验证`、`访问受限` 和 `处理失败` 都不会被断点续跑当成最终成功结果，下次继续时会重新尝试。
...
这里可能包含输入链接、进度、日志和 Selenium 浏览器缓存。多人共用电脑时，请注意该目录中的信息。macOS `0.5.2` 不再保存验证码账号或密码。
